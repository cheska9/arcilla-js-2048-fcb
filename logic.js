...
//if one of this variables value became true, it means the player already won
...
function slideLeft(){
	...
	for(let r=0; r<rows; r++){
		...
		for(let c = 0; c<columns; c++){
			...
			let num = board[r][c];
			updateTile(tile, num); //update color using updateTile functions
...
function slideUp(){
    for(let c=0; c<columns; c++){
        ...
        */
        ...
        // to update the id of the tile, and its color
        for(let r=0; r<rows; r++){
            board[r][c] = row[r];
...
function slideDown(){
    for(let c=0; c<columns; c++){
        ...
        row.reverse();
        row = slide(row);
        row.reverse(); //once done in merging we set it back to its original sequence through reversing again 
...
//this listen to when you touch the screen and assign the x coordinates of that touch
